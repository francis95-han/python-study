...
class Task(threading.Thread):
    def __init__(self,device_id,result):
        ...
        self.header = {"Host":"119.253.83.253:2080","user-agent":"Mozilla/5.0(Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.92 Safari/537.36"}
    ...
    def task_runner(self,deviceId,result):
        ...
        if requests.post(url=self.url,data=jsonstring,headers=self.header).ok:
            print("ok")
            self.result.enqueue("1")
...
def test():
    ...
    while i < len(qu):
        ...
        t = Task("asdas",results)
        Thread_list.append(t)                             #加入线程池，按需使用
        t.start()	
        i+=1
    #writeToCsv(results)
    print(results.size())
def writeToCsv(queue):
    path = "D:/test/abtest/power/"
